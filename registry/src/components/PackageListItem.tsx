    ...
    return (
        <div className="border p-4 rounded flex justify-between items-center">
            <div className="flex flex-col gap-2">
                <Link className="text-xl font-bold" to={`/packages/${pack.name}`}>{pack.name}</Link>
            ...
            <div className="mr-6">
                <Link className="py-2 px-3 rounded bg-black text-white text-lg"
                      to={`/packages/${pack.name}`}>View</Link>
            </div>
        </div>
    );
}